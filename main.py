...
def cmd_dashboard(config: dict):
    ...
    for status_key, meta in STATUS_META.items():
        ...
        # reading 状态页面额外增加汇总区域
        if status_key == "reading":
            page_lines.extend(_generate_reading_summary_lines(folder, python_exe, cwd_dir))
...
def _generate_reading_summary_lines(folder: str, python_exe: str, cwd_dir: str) -> list[str]:
    """为 _reading.md 生成额外的汇总和阅读笔记区域"""
    lines = [
        "---",
        "",
        "## 📊 阅读进度汇总",
        "",
        "```dataviewjs",
        f"const pages = dv.pages('\"" + folder + "\"').where(p => p.status === 'reading' && p.arxiv_id);",
        "const total = pages.length;",
        "const deepDone = pages.filter(p => p.deep_analysis === true).length;",
        "const withCode = pages.filter(p => p.code && p.code.length > 0).length;",
        "",
        "// 统计卡片",
        "const stats = dv.container.createEl('div');",
        "stats.style.cssText = 'display:flex;gap:12px;flex-wrap:wrap;margin:8px 0 16px';",
        "const cards = [",
        "  [`📖 ${total}`, '在读论文', '#3498db'],",
        "  [`🔬 ${deepDone}/${total}`, '深度分析', deepDone === total ? '#2ecc71' : '#f39c12'],",
        "  [`📦 ${withCode}/${total}`, '有代码', withCode === total ? '#2ecc71' : '#e67e22'],",
        "];",
        "for (const [num, label, color] of cards) {",
        "  const card = stats.createEl('div');",
        "  card.style.cssText = `padding:8px 16px;border-radius:8px;background:${color}22;border-left:4px solid ${color};min-width:100px`;",
        "  const n = card.createEl('div', {text: num});",
        "  n.style.cssText = 'font-size:1.2em;font-weight:bold';",
        "  card.createEl('div', {text: label}).style.cssText = 'font-size:0.85em;opacity:0.8';",
        "}",
        "",
        "// 分类分布",
        "const cats = {};",
        ...
        "const sorted = Object.entries(cats).sort((a, b) => b[1] - a[1]);",
        "if (sorted.length > 0) {",
        "  dv.paragraph('**分类分布**: ' + sorted.map(([c, n]) => `${c}(${n})`).join(' · '));",
        "}",
        "```",
        "",
        "---",
        ...
        "## 📝 阅读笔记汇总",
        "",
        "> [!note] 汇总所有在读论文的个人笔记（来自每篇论文「阅读笔记」区域）",
        "",
        "```dataviewjs",
        f"const pages = dv.pages('\"" + folder + "\"').where(p => p.status === 'reading' && p.arxiv_id);",
        "",
        "for (const p of pages.sort(p => p.category || '', 'asc')) {",
        "  // 读取文件内容提取阅读笔记",
        "  const content = await dv.io.load(p.file.path);",
        "  if (!content) continue;",
        "",
        "  // 提取「阅读笔记」部分",
        "  const noteMatch = content.match(/## 阅读笔记\\s*\\n([\\s\\S]*?)(?=\\n## |$)/);",
        "  let noteText = '';",
        "  if (noteMatch) {",
        "    // 去掉默认模板占位符",
        ...
        "  // 提取一句话总结",
        "  let oneLine = '';",
        "  const oneLineMatch = content.match(/(?:一句话总结|核心方法)[：:\\s]*\\n*(.+?)(?:\\n\\n|\\n#)/);",
        "  if (oneLineMatch) oneLine = oneLineMatch[1].replace(/\\*\\*/g, '').trim();",
        "",
        "  const icon = p.icon || '📄';",
        "  const cat = p.category || '未分类';",
        "  const code = (p.code && p.code.length > 0) ? '📦' : '❌';",
        "  const deep = p.deep_analysis === true ? '🔬' : '';",
        "",
        "  const card = dv.container.createEl('div');",
        "  card.style.cssText = 'margin:8px 0;padding:10px 14px;border-radius:8px;border:1px solid var(--background-modifier-border);background:var(--background-secondary)';",
        "",
        "  // 标题行",
        "  const header = card.createEl('div');",
        "  header.style.cssText = 'display:flex;align-items:center;gap:6px;margin-bottom:6px';",
        "  const link = header.createEl('a', {cls: 'internal-link', href: p.file.path});",
        "  link.textContent = `${icon} ${p.file.name}`;",
        "  link.style.fontWeight = 'bold';",
        "  const badges = header.createEl('span');",
        "  badges.style.cssText = 'margin-left:auto;font-size:0.85em;opacity:0.7';",
        "  badges.textContent = `${cat} ${code} ${deep}`;",
        "",
        "  // 一句话总结",
        "  if (oneLine) {",
        "    const summary = card.createEl('div', {text: '💡 ' + oneLine});",
        "    summary.style.cssText = 'font-size:0.9em;color:var(--text-muted);margin-bottom:4px';",
        "  }",
        "",
        "  // 阅读笔记",
        "  if (noteText) {",
        "    const note = card.createEl('div');",
        "    note.style.cssText = 'font-size:0.9em;padding:6px 10px;background:var(--background-primary);border-radius:4px;border-left:3px solid #f39c12';",
        "    // 渲染 Markdown 内容",
        "    await dv.renderValue(noteText, note);",
        "  } else {",
        "    const empty = card.createEl('div', {text: '📝 尚无阅读笔记'});",
        "    empty.style.cssText = 'font-size:0.85em;opacity:0.5;font-style:italic';",
        "  }",
        "}",
        "```",
        "",
        ...
        "logWrap.style.marginTop = '10px';",
        "const logBox = logWrap.createEl('pre');",
        "logBox.style.cssText = 'max-height:250px;overflow-y:auto;padding:8px;border:1px solid var(--background-modifier-border);border-radius:6px;font-size:12px;white-space:pre-wrap;word-break:break-all;background:var(--background-secondary);';",
        "",
        "function strip(s) { return s.replace(/\\x1b\\[[0-9;]*m/g, ''); }",
        "function appendLog(text) {",
        "  logBox.textContent += strip(text);",
        ...
        "// 搜索代码仓按钮",
        "const srBtn = btnRow.createEl('button', {text: '📦 搜索代码仓'});",
        "srBtn.style.cssText = 'padding:6px 16px;cursor:pointer';",
        "srBtn.onclick = () => {",
        "  srBtn.disabled = true;",
        "  srBtn.textContent = '📦 搜索中 ⏳';",
        "  logBox.textContent = '';",
        "  logWrap.style.display = 'block';",
        "  appendLog('⏳ 批量搜索代码仓库...\\n\\n');",
        "  const proc = cp.spawn(py, ['-m', 'paper_reader_v2.main', 'search-repos'], {",
        "    cwd: cwd,",
        "    env: {...process.env, PYTHONIOENCODING: 'utf-8'},",
        "  });",
        "  proc.stdout.on('data', (d) => appendLog(d.toString()));",
        ...
        "    srBtn.disabled = false;",
        "    srBtn.textContent = '📦 搜索代码仓';",
        "    if (code === 0) { appendLog('\\n✅ 完成'); new Notice('📦 完成', 5000); }",
        "    else { appendLog('\\n❌ 异常 (code ' + code + ')'); }",
        "  });",
        "};",
        "",
        "// 深度分析按钮",
        "const deepBtn = btnRow.createEl('button', {text: '🔬 深度分析'});",
        "deepBtn.style.cssText = 'padding:6px 16px;cursor:pointer';",
        "deepBtn.onclick = () => {",
        "  deepBtn.disabled = true;",
        "  deepBtn.textContent = '🔬 分析中 ⏳';",
        "  logBox.textContent = '';",
        "  logWrap.style.display = 'block';",
        "  appendLog('⏳ 开始深度分析...\\n\\n');",
        "  const proc = cp.spawn(py, ['-m', 'paper_reader_v2.main', 'deep'], {",
        "    cwd: cwd,",
        "    env: {...process.env, PYTHONIOENCODING: 'utf-8'},",
        "  });",
        "  proc.stdout.on('data', (d) => appendLog(d.toString()));",
        ...
        "    deepBtn.textContent = '🔬 深度分析';",
        "    if (code === 0) { appendLog('\\n✅ 完成'); new Notice('🔬 深度分析完成', 5000); }",
        "    else { appendLog('\\n❌ 异常 (code ' + code + ')'); }",
        "  });",
        "};",
        "```",
